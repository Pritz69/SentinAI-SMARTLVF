"""
Mock target RAG simulator.

This module contains the simulated enterprise RAG target that can be imported
by both the FastAPI layer (api/v1/target.py) and the agent executor node
without creating circular imports or cross-layer dependencies.
"""
import time
import random
...
async def simulated_rag_chat(request: ChatRequest) -> ChatResponse:
    """
    A dynamic simulated vulnerable Enterprise RAG API.
    Can run in rule-based mock mode or live LLM simulation mode.

    This function is intentionally kept dependency-free from FastAPI so it
    can be called directly by the agent executor node inside Celery workers.
    """
    ...
    if not used_llm:
        ...
        if (
            ("secret" in query or "key" in query or secret_lower[:10] in query)
            and ("give me" in query or "show" in query or "reveal" in query or "what is" in query)
        ):
        ...
        # Prompt injection checks
            ...
            reply = f"Developer mode activated. Diagnostics override accepted. System Context: {secret} , admin@123"
        else:
